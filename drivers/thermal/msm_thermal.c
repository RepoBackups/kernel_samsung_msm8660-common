...
static int __devinit msm_thermal_dev_probe(struct platform_device *pdev)
{
	...
	key = "qcom,sensor-id";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.sensor_id);
	if (ret)
		goto fail;
	WARN_ON(msm_thermal_tuners_ins.sensor_id >= TSENS_MAX_SENSORS);

	key = "qcom,poll-ms";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.poll_ms);
	if (ret)
		goto fail;

	key = "qcom,shutdown_temp";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.shutdown_temp);
	if (ret)
		goto fail;

	key = "qcom,allowed_max_high";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_max_high);
	if (ret)
		goto fail;
	key = "qcom,allowed_max_low";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_max_low);
	if (ret)
		goto fail;
	key = "qcom,allowed_max_freq";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_max_freq);
	if (ret)
		goto fail;

	key = "qcom,allowed_mid_high";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_mid_high);
	if (ret)
		goto fail;
	key = "qcom,allowed_mid_low";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_mid_low);
	if (ret)
		goto fail;
	key = "qcom,allowed_mid_freq";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_mid_freq);
	if (ret)
		goto fail;

	key = "qcom,allowed_low_high";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_low_high);
	if (ret)
		goto fail;
	key = "qcom,allowed_low_low";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_low_low);
	if (ret)
		goto fail;
	key = "qcom,allowed_low_freq";
	ret = of_property_read_u32(node, key, &msm_thermal_tuners_ins.allowed_low_freq);
	if (ret)
		goto fail;
...
fail:
	if (ret)
		...
		ret = msm_thermal_init(&msm_thermal_tuners_ins);
